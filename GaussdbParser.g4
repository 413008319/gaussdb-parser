parser grammar GaussdbParser;


options { tokenVocab = GaussdbLexer;
}
...
stmt
   ...
   | gaussdb_create_node
   | gaussdb_create_node_group
   | gaussdb_create_resource_pool
   | gaussdb_create_resource_label
   | gaussdb_create_security_label
   | gaussdb_create_synonym
   | gaussdb_create_weak_password_dic
   | gaussdb_purge_stmt
   | gaussdb_replacestmt
   | gaussdb_shutdown_stmt
   | gaussdb_show_events_stmt
   ;
...
callstmt
   : CALL? func_application into_clause?
   ;
...
alteroptroleelem
   : (PASSWORD | IDENTIFIED BY) (sconst | NULL_P | ) (EXPIRED | DISABLE_P)?
   | (ENCRYPTED | UNENCRYPTED) (PASSWORD | IDENTIFIED BY) sconst (EXPIRED | DISABLE_P)?
   | INHERIT
   | CONNECTION LIMIT signediconst
   | RESOURCE POOL sconst
   | VALID (UNTIL | BEGIN_P) sconst
   | USER GROUP_P sconst
   | PERM SPACE_P sconst
   | TEMP SPACE_P sconst
   | SPILL SPACE_P sconst
   | SPILL SPACE_P sconst
   | NODE GROUP_P any_name
   | IN_P? ROLE role_list
   | IN_P GROUP_P role_list
   | ADMIN role_list
   | USER role_list
   | SYSID a_expr
   | ACCOUNT (LOCK_P | UNLOCK_P)
   | PGUSER
   | DEFAULT TABLESPACE any_name
   | PROFILE DEFAULT
   | PROFILE any_name
   | identifier
   ;
...
createschemastmt
   : CREATE SCHEMA (IF_P NOT EXISTS)? (optschemaname? AUTHORIZATION rolespec | colid) (WITH BLOCKCHAIN)?  optschemaeltlist
   ;
...
set_rest_more
   ...
   | SCHEMA sconst
   | NAMES opt_encoding?
   | ROLE nonreservedword_or_sconst (PASSWORD var_value)?
   | SESSION AUTHORIZATION nonreservedword_or_sconst (PASSWORD var_value)?
...
variableshowstmt
   : SHOW (var_name | TIME ZONE | TRANSACTION ISOLATION LEVEL | SESSION AUTHORIZATION | ALL | CURRENT_SCHEMA)
   ;
...
altertablestmt
   : ALTER TABLE gaussdb_online? (IF_P EXISTS)? relation_expr (alter_table_cmds | partition_cmd | gaussdb_truncate_table_partition)
   | ALTER TABLE ALL IN_P TABLESPACE name (OWNED BY role_list)? SET TABLESPACE name opt_nowait?
   | ALTER INDEX (IF_P EXISTS)? qualified_name (alter_table_cmds | index_partition_cmd | gaussdb_alter_index_ops_set?)
...
alter_table_cmd
   ...
   //gaussdb新加
   | GSIWAITALL (IN_P a_expr MODE)?
   | gaussdb_to_group
   | ADD_P NODE gaussdb_nodenamelist
   | DELETE_P NODE gaussdb_nodenamelist
   | UPDATE SLICE LIKE a_expr
   | ENCRYPTION KEY ROTATION
   | AUTO_INCREMENT EQUAL? a_expr
   | gaussdb_comment
   | (DEFAULT? CHARACTER SET | CHARSET EQUAL? a_expr )? (DEFAULT? COLLATE EQUAL? a_expr)?
   | gaussdb_ilm_add_policy
   | ALTER opt_column? colid SET STATISTICS PERCENT_P? Integral
   | ADD_P STATISTICS OPEN_PAREN OPEN_PAREN colid (COMMA colid)* CLOSE_PAREN CLOSE_PAREN
   | DELETE_P STATISTICS OPEN_PAREN OPEN_PAREN colid (COMMA colid)* CLOSE_PAREN CLOSE_PAREN
   | MODIFY colid typename COMMENT comment_text (COMMENT comment_text)*
   | MODIFY colid typename
   | MODIFY colid (CONSTRAINT name)? NOT NULL ENABLE_P?
   | MODIFY colid (CONSTRAINT name)? NULL
   | MODIFY OPEN_PAREN colid typename (COMMA colid typename)* CLOSE_PAREN
   | MODIFY OPEN_PAREN colid (CONSTRAINT name)? NOT NULL ENABLE_P? (COMMA (CONSTRAINT name)? NOT NULL ENABLE_P?)* CLOSE_PAREN
   | MODIFY OPEN_PAREN colid (CONSTRAINT name)? NULL (COMMA (CONSTRAINT name)? NULL)* CLOSE_PAREN
   | htap_action (  OPEN_PAREN colid (COMMA colid)* CLOSE_PAREN )? ( PRIORITY ( HIGH | LOW | NONE ) )?
   ;
...
copystmt
   : COPY opt_binary? qualified_name opt_column_list? copy_from opt_program? copy_file_name copy_delimiter? (WITHOUT ESCAPING)? opt_with? copy_options where_clause?
   | COPY OPEN_PAREN preparablestmt CLOSE_PAREN TO opt_program? copy_file_name (WITHOUT ESCAPING)? opt_with? copy_options
   ;
...
copy_options
   : copy_opt_list
   | OPEN_PAREN copy_opt_item (COMMA copy_opt_item)* CLOSE_PAREN
...
copy_opt_item
   ...
   | CSV
   | HEADER_P var_value?
   | FILEHEADER var_value?
   | USEEOF var_value?
   | NOESCAPING var_value?
   | IGNORE_EXTRA_DATA var_value?
   | FILL_MISSING_FIELDS var_value?
   | COMPATIBLE_ILLEGAL_CHARS var_value?
   | QUOTE opt_as? sconst
   ...
   | ENCODING sconst
   | FIXED FORMATTER opt_column_list
   | FORMAT var_value
   | DATE_FORMAT sconst
   | TIME_FORMAT sconst
   | TIMESTAMP_FORMAT sconst
   | SMALLDATETIME_FORMAT sconst
   | OIDS var_value
   ;
...
createstmt
   : CREATE opttemp? TABLE (IF_P NOT EXISTS)? qualified_name (
      (OPEN_PAREN opttableelementlist? CLOSE_PAREN | LIKE qualified_name)
            optinherit? gaussdb_table_option_list? gaussdb_htap_option? optpartitionspec? table_access_method_clause? optwith? oncommitoption? gaussdb_ilm_add_policy? opttablespace?
            gaussdb_distribute?
            gaussdb_to_group?
...
columnDef
   : colid typename (CHARACTER SET | CHARACTER EQUAL? a_expr)? opt_collate? gaussdb_create_generic_position? create_generic_options? colquallist
   ;
...
colconstraintelem
   : NOT NULL_P
   | NULL_P
   | UNIQUE KEY? opt_definition? optconstablespace?
   ...
   | NOCOLVIEW
   | ON UPDATE a_expr
   | ENCRYPTED WITH OPEN_PAREN COLUMN_ENCRYPTION_KEY EQUAL a_expr COMMA ENCRYPTION_TYPE EQUAL a_expr  CLOSE_PAREN
   ;
...
constraintelem
   : CHECK OPEN_PAREN a_expr CLOSE_PAREN constraintattributespec gaussdb_comment_list
   | UNIQUE access_method_clause? (OPEN_PAREN columnlist CLOSE_PAREN opt_c_include? opt_definition? optconstablespace? constraintattributespec gaussdb_comment_list | existingindex constraintattributespec gaussdb_comment_list)
   | PRIMARY KEY access_method_clause? (OPEN_PAREN columnlist CLOSE_PAREN opt_c_include? opt_definition? optconstablespace? constraintattributespec gaussdb_comment_list | existingindex constraintattributespec gaussdb_comment_list)
   | EXCLUDE access_method_clause? OPEN_PAREN exclusionconstraintlist CLOSE_PAREN opt_c_include? opt_definition? optconstablespace? exclusionwhereclause? constraintattributespec gaussdb_comment_list
   | (FOREIGN KEY OPEN_PAREN columnlist CLOSE_PAREN)? REFERENCES qualified_name opt_column_list? key_match? key_actions? constraintattributespec gaussdb_comment_list
   | ON UPDATE a_expr constraintattributespec gaussdb_comment_list
   | GENERATED ALWAYS AS OPEN_PAREN a_expr CLOSE_PAREN STORED? constraintattributespec gaussdb_comment_list
   | GENERATED ALWAYS AS OPEN_PAREN a_expr CLOSE_PAREN STORED? constraintattributespec gaussdb_comment_list
   | AUTO_INCREMENT constraintattributespec gaussdb_comment_list
   | COMMENT comment_text
   | COLVIEW constraintattributespec gaussdb_comment_list
   | NOCOLVIEW constraintattributespec gaussdb_comment_list
   ;
...
columnElem
   : ( colid column_length? | OPEN_PAREN a_expr CLOSE_PAREN ) opt_asc_desc?
   ;

column_length
    : OPEN_PAREN numericonly (COMMA numericonly)* CLOSE_PAREN
    ;
...
partitionspec
   : PARTITION BY colid COLUMNS? ( PARTITIONS numeric)? OPEN_PAREN part_params CLOSE_PAREN
   ( gaussdb_partition_less_than_item_list
    |gaussdb_partition_start_end_item_list
    |gaussdb_partition_hash_item
    |gaussdb_partition_list_item)?
   ( ( ENABLE_P | DISABLE_P ) ROW MOVEMENT )?
   ;
...
create_as_target
   : qualified_name opt_column_list? table_access_method_clause? gaussdb_engine_list? optwith? oncommitoption? gaussdb_ilm_add_policy? opttablespace?  gaussdb_to_group?
       (DISTRIBUTE BY (
                 REPLICATION
               | HASH? OPEN_PAREN colid (COMMA colid)* CLOSE_PAREN )
          )?
        optpartitionspec?
   ;
...
create_inc_mat_viewstmt
   : CREATE INCREMENTAL  MATERIALIZED VIEW (IF_P NOT EXISTS)? create_mv_target AS selectstmt opt_with_data?
   ;
...
createtablespacestmt
   : CREATE TABLESPACE name  opttablespaceowner? (RELATIVE_P)? LOCATION sconst (MAXSIZE sconst)? opt_reloptions?
   ;
...
createforeigntablestmt
   : CREATE FOREIGN TABLE qualified_name OPEN_PAREN opttableelementlist? CLOSE_PAREN optinherit? SERVER name create_generic_options? gaussdb_create_foreigntable_extend
   | CREATE FOREIGN TABLE IF_P NOT EXISTS qualified_name OPEN_PAREN opttableelementlist? CLOSE_PAREN optinherit? SERVER name create_generic_options? gaussdb_create_foreigntable_extend
   | CREATE FOREIGN TABLE qualified_name PARTITION OF qualified_name opttypedtableelementlist? partitionboundspec SERVER name create_generic_options? gaussdb_create_foreigntable_extend
   | CREATE FOREIGN TABLE IF_P NOT EXISTS qualified_name PARTITION OF qualified_name opttypedtableelementlist? partitionboundspec SERVER name create_generic_options? gaussdb_create_foreigntable_extend
   ;
...
createpolicystmt
   : CREATE (ROW LEVEL SECURITY)? POLICY name ON qualified_name rowsecuritydefaultpermissive? rowsecuritydefaultforcmd? rowsecuritydefaulttorole? rowsecurityoptionalexpr? rowsecurityoptionalwithcheck?
   ;
...
createtrigstmt
   : CREATE gaussdb_or_replace? TRIGGER name triggeractiontime triggerevents ON qualified_name triggerreferencing? triggerforspec? triggerwhen? (EXECUTE function_or_procedure func_name OPEN_PAREN triggerfuncargs CLOSE_PAREN | trigger_anonymous_block)
   | CREATE CONSTRAINT TRIGGER name AFTER triggerevents ON qualified_name optconstrfromtable? constraintattributespec FOR EACH ROW triggerwhen? (EXECUTE function_or_procedure func_name OPEN_PAREN triggerfuncargs CLOSE_PAREN | trigger_anonymous_block)
   ;
...
definestmt
   ...
   | CREATE TYPE_P any_name AS TABLE OF identifier
...
dropstmt
   : DROP object_type_any_name IF_P EXISTS any_name_list opt_drop_behavior? gaussdb_PURGE?
   | DROP object_type_any_name any_name_list opt_drop_behavior? gaussdb_PURGE?
   ...
   | DROP AUDIT POLICY (IF_P EXISTS)? any_name_list opt_drop_behavior?
   | DROP MASKING POLICY  (IF_P EXISTS)? any_name_list opt_drop_behavior?
   | DROP CLIENT MASTER KEY (IF_P EXISTS)? any_name_list opt_drop_behavior?
   | DROP COLUMN ENCRYPTION KEY (IF_P EXISTS)? any_name_list opt_drop_behavior?
   | DROP PUBLIC? DATABASE LINK (IF_P EXISTS)? any_name_list opt_drop_behavior?
   | DROP PUBLIC? SYNONYM (IF_P EXISTS)? any_name_list opt_drop_behavior?
   | DROP DIRECTORY (IF_P EXISTS)? any_name_list opt_drop_behavior?
   | DROP GLOBAL CONFIGURATION (IF_P EXISTS)? any_name_list opt_drop_behavior?
   | DROP MODEL (IF_P EXISTS)? any_name_list opt_drop_behavior?
   | DROP PACKAGE (IF_P EXISTS)? any_name_list opt_drop_behavior?
   | DROP SECURITY LABEL (IF_P EXISTS)? any_name_list opt_drop_behavior?
   | DROP RESOURCE LABEL  (IF_P EXISTS)? any_name_list opt_drop_behavior?
   | DROP RESOURCE POOL  (IF_P EXISTS)? any_name_list opt_drop_behavior?
   | DROP NODE (IF_P EXISTS)? any_name_list (WITH OPEN_PAREN any_name (COMMA any_name)* CLOSE_PAREN )?
   | DROP NODE GROUP_P  (IF_P EXISTS)? any_name_list (TO ELASTIC GROUP_P )?
...
truncatestmt
   : TRUNCATE opt_table? relation_expr_list opt_restart_seqs? opt_drop_behavior? PURGE?
   ;
...
cursorstmt
   : CURSOR any_name  BINARY?   (NO SCROLL)?  ((WITH | WITHOUT) HOLD)? FOR selectstmt
   ;
...
privilege_target
   ...
   | COLUMN_ENCRYPTION_KEY name_list
   | CLIENT_MASTER_KEY name_list
   ;
...
gaussdb_opt_distribute
   : (DISTRIBUTE BY HASH | CONTAINING) OPEN_PAREN colid (COMMA colid)* CLOSE_PAREN
    ;
...
createfunctionstmt
   : CREATE opt_or_replace? (FUNCTION | PROCEDURE) func_name func_args_with_defaults?
     (
        RETURNS (func_return DETERMINISTIC?| TABLE OPEN_PAREN table_func_column_list CLOSE_PAREN)
     )?
     (
        RETURN func_return DETERMINISTIC?
     )?
     createfunc_opt_list
   ;
...
func_arg_with_default
   : func_arg ((DEFAULT | COLON_EQUALS | EQUAL) a_expr)?
   ;
...
reindexstmt
   : REINDEX reindex_target_type opt_concurrently? qualified_name FORCE?
   | REINDEX reindex_target_multitable opt_concurrently? name FORCE?
   | REINDEX OPEN_PAREN reindex_option_list CLOSE_PAREN reindex_target_type opt_concurrently? qualified_name FORCE?
   | REINDEX OPEN_PAREN reindex_option_list CLOSE_PAREN reindex_target_multitable opt_concurrently? name FORCE?
   ;
...
renamestmt
   ...
   | ALTER TABLE gaussdb_online? relation_expr RENAME TO name
   | ALTER TABLE gaussdb_online? IF_P EXISTS relation_expr RENAME TO name
   ...
   | ALTER TABLE gaussdb_online? relation_expr RENAME opt_column? name TO name
   | ALTER TABLE gaussdb_online? IF_P EXISTS relation_expr RENAME opt_column? name TO name
   ...
   | RENAME (TABLE|TABLES) relation_expr TO name
...
alterobjectschemastmt
   ...
   | ALTER TABLE gaussdb_online? relation_expr SET SCHEMA name
   | ALTER TABLE gaussdb_online? IF_P EXISTS relation_expr SET SCHEMA name
...
transactionstmt
   ...
   | COMMIT PREPARED sconst (WITH a_expr)?
...
clusterstmt
   ...
   | CLUSTER opt_verbose? name PARTITION OPEN_PAREN gaussdb_partition_name CLOSE_PAREN (USING gaussdb_index_name)?
   ;
...
explainplanstmt
   : EXPLAIN PLAN (SET STATEMENT_ID EQUAL a_expr )? FOR explainablestmt
   ;
...
insertstmt
   : opt_with_clause? INSERT (IGNORE | ALL | FIRST)? gaussdb_insert_when_clause? INTO insert_target insert_rest opt_on_conflict? returning_clause?
   ;
...
merge_insert_clause
   : WHEN NOT MATCHED (AND a_expr)? THEN? INSERT (OPEN_PAREN insert_column_list CLOSE_PAREN)? values_clause  where_clause?
   ;

merge_update_clause
   : WHEN MATCHED (AND a_expr)? THEN? UPDATE SET set_clause_list where_clause?
   ;
...
lockstmt
   : LOCK_P opt_table? relation_expr_list opt_lock? opt_nowait? CANCELABLE?
   ;

lockbucketsstmt
   : LOCK_P BUCKETS OPEN_PAREN opt_bucketlist CLOSE_PAREN opt_lock?  CANCELABLE?
   ;

markbucketsstmt
   : MARK BUCKETS  OPEN_PAREN opt_bucketlist CLOSE_PAREN FINISH FROM any_name TO any_name
   ;
...
updatestmt
   : opt_with_clause? UPDATE relation_expr_opt_alias SET set_clause_list from_clause? where_or_current_clause? gaussdb_sort_clause? gaussdb_limit_clause? returning_clause?
   ;
...
set_clause
   ...
   | OPEN_PAREN? set_target_list CLOSE_PAREN? EQUAL selectstmt
...
values_clause
   : (VALUES|VALUE_P) OPEN_PAREN expr_list CLOSE_PAREN (COMMA OPEN_PAREN expr_list CLOSE_PAREN)*
   ;
...
gaussdb_create_generic_position
   : POSITION OPEN_PAREN a_expr COMMA a_expr CLOSE_PAREN
   ;

//创建 foreigntable 扩展语句
gaussdb_create_foreigntable_extend
   : (WRITE ONLY | READ ONLY)?
   | (WITH gaussdb_error_table_name | LOG INTO gaussdb_error_table_name)?
   | (REMOTE LOG sconst)?
   | (PER NODE REJECT LIMIT sconst)?
   | gaussdb_to_group?
   ;

gaussdb_error_table_name
   : a_expr
   ;

gaussdb_nodenamelist
   : OPEN_PAREN a_expr (COMMA a_expr)* CLOSE_PAREN
   ;
...
gaussdb_to_group
   : TO (GROUP_P gaussdb_groupname | NODE gaussdb_nodenamelist)
   ;

gaussdb_limit_clause
   : limit_clause?
   ;
...
gaussdb_truncate_table_partition
   : TRUNCATE gaussdb_partition_extended_names (UPDATE GLOBAL INDEX)?
   ;

gaussdb_partition_extended_names
   : (PARTITION | PARTITIONS) (
       OPEN_PAREN? gaussdb_partition_name (COMMA gaussdb_partition_name)* CLOSE_PAREN?
       | FOR OPEN_PAREN? gaussdb_partition_key_value (COMMA gaussdb_partition_key_value)* CLOSE_PAREN?
   )
   ;

gaussdb_partition_name
   : a_expr
   ;

gaussdb_new_partition_name
   : a_expr
   ;

gaussdb_partition_key_value
   : a_expr
   ;
...
gaussdb_comment_list
   : (gaussdb_comment)*
   ;

gaussdb_comment
   : COMMENT EQUAL? comment_text
   ;
...
gaussdb_rebuild_clause
    : REBUILD (PARTITION gaussdb_partition_name )?
    ;

gaussdb_alter_index_partitioning
    : gaussdb_modify_index_partition
    | gaussdb_rename_index_partition
    | gaussdb_move_index_partition
    ;

gaussdb_modify_index_partition
    : (MODIFY PARTITION gaussdb_partition_name)?  UNUSABLE
    ;

gaussdb_rename_index_partition
    : RENAME (PARTITION gaussdb_partition_name ) TO gaussdb_new_partition_name
    ;

gaussdb_move_index_partition
    : MOVE (PARTITION gaussdb_partition_name ) TABLESPACE gaussdb_new_partition_name
    ;
//[ OFFLINE | [ ONLINE [ WITH ( { online_parameter = value } [, ... ] ) ] ] ]
gaussdb_online
    : OFFLINE
    | ONLINE (opt_reloptions)?
    ;


gaussdb_index_partition_name
   : a_expr
   ;

gaussdb_partition_value
   : a_expr
   ;

gaussdb_index_partition_tablespace
   : a_expr
   ;

gaussdb_index_partition_list
    : OPEN_PAREN gaussdb_index_partition (COMMA gaussdb_index_partition)* CLOSE_PAREN
    ;

gaussdb_index_partition
    : PARTITION gaussdb_index_partition_name (FOR ( gaussdb_partition_name | gaussdb_partition_value_list ) )? (TABLESPACE gaussdb_index_partition_tablespace)?  gaussdb_index_subpartition_list?
    ;

gaussdb_partition_value_list
    : OPEN_PAREN gaussdb_partition_value (COMMA gaussdb_partition_value)* CLOSE_PAREN
    ;

gaussdb_index_subpartition_list
    : OPEN_PAREN gaussdb_index_subppartition (COMMA gaussdb_index_subppartition)* CLOSE_PAREN
    ;

gaussdb_index_subppartition
    : SUBPARTITION gaussdb_index_partition_name (FOR ( gaussdb_partition_name | gaussdb_partition_value_list ) )? (TABLESPACE gaussdb_index_partition_tablespace)?
    ;

gaussdb_connect_clause
   : (START WITH)? CONNECT BY NOCYCLE? a_expr ( ORDER SIBLINGS BY a_expr)?
   ;

gaussdb_engine_list
   : gaussdb_engine_item (COMMA? gaussdb_engine_item)?
   ;

gaussdb_engine_item
   : ENGINE EQUAL? (INNODB | INNODB1 | INNODB2 )
   ;

gaussdb_table_option_list
   : gaussdb_table_option (COMMA gaussdb_table_option)?
   ;

gaussdb_table_option
   : gaussdb_comment
   | AUTO_INCREMENT EQUAL? a_expr
   | DEFAULT? CHARACTER SET
   | CHARSET EQUAL? a_expr
   | DEFAULT? COLLATE EQUAL? a_expr
   | gaussdb_engine_item
   ;
...
gaussdb_distribute
   : DISTRIBUTE BY (
          REPLICATION
        | HASH OPEN_PAREN colid (COMMA colid)* CLOSE_PAREN
        | MURMURHASH OPEN_PAREN a_expr CLOSE_PAREN
        | KMEANS OPEN_PAREN colid CLOSE_PAREN (L2 | COSINE | HAMMING)
        | RANGE OPEN_PAREN colid (COMMA colid)* CLOSE_PAREN ( SLICE REFERENCES colid
                                                                | gaussdb_slice_less_than_item_list
                                                                | gaussdb_slice_start_end_item_list
                                                            )
        | LIST OPEN_PAREN colid (COMMA colid)* CLOSE_PAREN ( SLICE REFERENCES colid
                                                                | gaussdb_slice_values_item_list
                                                            )
   )
   ;

gaussdb_slice_less_than_item_list
   : OPEN_PAREN gaussdb_slice_less_than_item (COMMA gaussdb_slice_less_than_item)* CLOSE_PAREN
   ;

gaussdb_slice_less_than_item
   : SLICE any_name VALUES LESS THAN OPEN_PAREN ( MAXVALUE | a_expr ) (COMMA (MAXVALUE | a_expr))* CLOSE_PAREN  ( DATANODE any_name | OPEN_PAREN any_name (COMMA any_name)? CLOSE_PAREN )?
   ;

gaussdb_slice_start_end_item_list
   : OPEN_PAREN gaussdb_slice_start_end_item (COMMA gaussdb_slice_start_end_item)* CLOSE_PAREN
   ;

gaussdb_slice_start_end_item
   : SLICE any_name (
          START OPEN_PAREN a_expr CLOSE_PAREN END_P OPEN_PAREN a_expr CLOSE_PAREN EVERY OPEN_PAREN a_expr CLOSE_PAREN
        | START OPEN_PAREN a_expr CLOSE_PAREN END_P OPEN_PAREN ( MAXVALUE | a_expr ) CLOSE_PAREN
        | START OPEN_PAREN a_expr CLOSE_PAREN
        | END_P OPEN_PAREN ( MAXVALUE | a_expr ) CLOSE_PAREN
   )
   ;

gaussdb_slice_values_item_list
   : OPEN_PAREN gaussdb_slice_values_item_item (COMMA gaussdb_slice_values_item_item)* CLOSE_PAREN
   ;

gaussdb_slice_values_item_item
   : SLICE any_name VALUES OPEN_PAREN ( DEFAULT | a_expr ) (COMMA (DEFAULT | a_expr))* CLOSE_PAREN  ( DATANODE any_name | OPEN_PAREN any_name (COMMA any_name)? CLOSE_PAREN )?
   ;

gaussdb_ilm_add_policy
   : ILM ADD_P POLICY ROW STORE COMPRESS ADVANCED ROW AFTER numericonly (DAY_P | MONTH_P | YEAR_P) OF NO MODIFICATION (ON OPEN_PAREN a_expr CLOSE_PAREN)?
   ;
...
body
    : BEGIN_P seq_of_statements (EXCEPTION exception_handler+)? END_P label_name?
    ;
...
seq_of_statements
    : (pragma_declaration* plsql_statement (SEMI | EOF)
        | label_declaration
        //预编译，gaussdb不支持？
        //| selection_directive
      )+
...
label_declaration
    : ltp1 = LESS_LESS label_name GREATER_GREATER
    ;
...
pragma_declaration
    : PRAGMA (
        ...
        | EXCEPTION_INIT OPEN_PAREN exception_name COMMA numeric_negative CLOSE_PAREN
        | INLINE_P OPEN_PAREN id1 = identifier COMMA a_expr CLOSE_PAREN
        | RESTRICT_REFERENCES OPEN_PAREN (identifier | DEFAULT) (COMMA identifier)+ CLOSE_PAREN
        | DEPRECATE OPEN_PAREN identifier ( COMMA anysconst)? CLOSE_PAREN
    ) SEMI
    ;
...
exception_declaration
    ...
    ;
default_value_part
    :((DEFAULT | COLON_EQUALS | EQUAL) a_expr)?
    ;

// incorporates constant_declaration
variable_declaration
    : identifier CONSTANT? func_type (NOT? NULL_P)? default_value_part? SEMI
    ;

subtype_declaration
    : SUBTYPE identifier IS func_type (RANGE a_expr DOT_DOT a_expr)? (NOT NULL_P)? SEMI
    ;

// cursor_declaration incorportates curscursor_body and cursor_spec

cursor_declaration
    : CURSOR identifier (OPEN_PAREN parameter_spec (COMMA parameter_spec)* CLOSE_PAREN)? (RETURN func_type)? (
        IS selectstmt
    )? SEMI
    ;

parameter_spec
    : parameter_name (IN_P | OUT_P | INOUT | NOCOPY)* func_type? default_value_part?
    ;

ref_cursor_type_def
    : REF CURSOR (RETURN func_type)?
    ;

type_declaration
    : TYPE_P identifier IS (
    table_type_def
    | varray_type_def
    | record_type_def
    | ref_cursor_type_def) SEMI
    ;

table_type_def
    : TABLE OF func_type (NOT NULL_P)? table_indexed_by_part?
    ;

table_indexed_by_part
    : (idx1 = INDEXED | idx2 = INDEX) BY func_type
    ;
...
varray_type_def
    : (VARRAY | VARYING? ARRAY) OPEN_PAREN a_expr CLOSE_PAREN OF func_type (NOT NULL_P)?
    ;

record_type_def
    : RECORD OPEN_PAREN field_spec (COMMA field_spec)* CLOSE_PAREN
    ;

field_spec
    : colid func_type? (NOT NULL_P)? default_value_part?
    ;

procedure_spec
    : PROCEDURE identifier (OPEN_PAREN parameter_spec ( COMMA parameter_spec)* CLOSE_PAREN)? (
...
accessible_by_clause
    : ACCESSIBLE BY OPEN_PAREN accessor (COMMA accessor)* CLOSE_PAREN
    ;

accessor
    : unitKind = (FUNCTION | PROCEDURE | PACKAGE | TRIGGER | TYPE_P) any_name
    ;

function_spec
    : FUNCTION identifier (OPEN_PAREN parameter_spec ( COMMA parameter_spec)* CLOSE_PAREN)? RETURN func_type (
        DETERMINISTIC
        | PIPELINED
...
partition_by_clause
    : OPEN_PAREN PARTITION a_expr BY (ANY | (HASH | RANGE | LIST) paren_column_list) streaming_clause? CLOSE_PAREN
    ;

streaming_clause
    : (ORDER | CLUSTER) a_expr BY paren_column_list
    ;
...
procedure_body
    : PROCEDURE identifier (OPEN_PAREN parameter_spec ( COMMA parameter_spec)* CLOSE_PAREN)? (
        accessible_by_clause
        | PARALLEL_ENABLE
    )* (IS | AS) DECLARE? seq_of_declare_specs? body SEMI
    ;
...
function_body
    : FUNCTION identifier (OPEN_PAREN parameter_spec ( COMMA parameter_spec)* CLOSE_PAREN)? RETURN func_type (
        ...
        ( (IS | AS) (IS | AS) DECLARE? seq_of_declare_specs? body)
        | (PIPELINED | AGGREGATE) USING implementation_type_name
    ) SEMI
    ;

result_cache_clause
    : RESULT_CACHE relies_on_part? (OPEN_PAREN MODE (DEFAULT | FORCE ) CLOSE_PAREN)?
    ;

relies_on_part
    : RELIES_ON OPEN_PAREN qualified_name (COMMA qualified_name)* CLOSE_PAREN
    ;
...
open_statement
    : OPEN cursor_name (OPEN_PAREN expressions? CLOSE_PAREN)?
    ;

fetch_statement
    : FETCH cursor_name (
        it1 = INTO variable_or_collection (COMMA variable_or_collection)*
        | BULK COLLECT INTO variable_or_collection (COMMA variable_or_collection)* (
...
collection_expression
    : any_name OPEN_PAREN expression CLOSE_PAREN (general_element)*
    ;

open_for_statement
    : OPEN a_expr FOR (selectstmt | expression) using_clause?
    ;

collection_method_call // collection methods invocation that could be used as a statement
    : expression DOT (
        (DELETE_P | EXTEND) (OPEN_PAREN index += expression (COMMA index += expression)* CLOSE_PAREN)?
        | TRIM (OPEN_PAREN index += expression CLOSE_PAREN)?
    )
    ;
...
pipe_row_statement
    : PIPE ROW OPEN_PAREN expression CLOSE_PAREN
    ;

forall_statement
    : FORALL any_name IN_P bounds_clause (SAVE EXCEPTIONS)? data_manipulation_language_statements
    ;
...
if_statement
    : IF_P condition THEN seq_of_statements elsif_part* else_part? END_P IF_P
    ;
...
loop_statement
    : label_declaration? (WHILE condition | FOR cursor_loop_param)? LOOP seq_of_statements END_P LOOP label_name?
    ;

// Loop Specific Clause
cursor_loop_param
    : any_name IN_P REVERSE? lower_bound range_separator = DOT_DOT upper_bound
    | any_name IN_P (cursor_name (OPEN_PAREN expressions? CLOSE_PAREN)? | OPEN_PAREN selectstmt CLOSE_PAREN)
    ;
...
assignment_statement
    : (general_element
        //| bind_variable //Pro*C/C++ indicator variables 不用支持？
    ) COLON_EQUALS a_expr
    ;

general_element
    : any_name
    | OPEN_PAREN general_element CLOSE_PAREN
    ;

continue_statement
    : CONTINUE_P label_name? (WHEN condition)?
    ;

// Expression & Condition
condition
    : expression
    | JSON_EQUAL OPEN_PAREN expressions CLOSE_PAREN
    ;
...
cursor_expression
    : CURSOR OPEN_PAREN selectstmt CLOSE_PAREN
    ;
...
logical_operation
    : (
        ...
        | INFINITE
        | EMPTY_
        | OF TYPE_P? OPEN_PAREN ONLY? func_type (COMMA func_type)* CLOSE_PAREN
        | JSON (FORMAT JSON)? (STRICT_P | LAX)? ((WITH | WITHOUT) UNIQUE KEYS)?
    )
    ;
...
concatenation
    ...
    | concatenation op = (STAR | SOLIDUS | MOD) concatenation
    | concatenation op = (PLUS | MINUS) concatenation
    | concatenation BAR BAR concatenation
    | concatenation COLLATE any_name
    ;
...
in_elements
    : OPEN_PAREN selectstmt CLOSE_PAREN
    | OPEN_PAREN concatenation (COMMA concatenation)* CLOSE_PAREN
    | constant
    | general_element
    ;
...
interval_expression
    : DAY_P (OPEN_PAREN concatenation CLOSE_PAREN)? TO SECOND_P (OPEN_PAREN concatenation CLOSE_PAREN)?
    | YEAR_P (OPEN_PAREN concatenation CLOSE_PAREN)? TO MONTH_P
    | concatenation (SECOND_P | DAY_P | MONTH_P | YEAR_P)
    ;

model_expression
    : unary_expression (OPEN_BRACKET model_expression_element CLOSE_BRACKET)?
    ;
...
single_column_for_loop
    : FOR any_name (
        IN_P OPEN_PAREN expressions? CLOSE_PAREN
...
multi_column_for_loop
    : FOR paren_column_list IN_P OPEN_PAREN (selectstmt | OPEN_PAREN expressions? CLOSE_PAREN) CLOSE_PAREN
    ;

unary_expression
    : (MINUS | PLUS) unary_expression
    ...
    | unary_expression DOT (
        (COUNT | FIRST_P | LAST_P | LIMIT)
        | (EXISTS | NEXT | PRIOR) OPEN_PAREN index += expression CLOSE_PAREN
    )
...
implicit_cursor_expression
    : SQL_P (
        PERCENT_BULK_ROWCOUNT OPEN_PAREN index = expression CLOSE_PAREN
        | PERCENT_BULK_EXCEPTIONS (DOT COUNT | OPEN_PAREN expression CLOSE_PAREN DOT (ERROR_INDEX | ERROR_CODE))
    )
    ;
...
simple_case_statement
    : label_declaration? ck1 = CASE expression simple_case_when_part+ case_else_part? END_P CASE? label_name?
    ;
...
searched_case_statement
    : label_declaration? ck1 = CASE searched_case_when_part+ case_else_part? END_P CASE? label_name?
    ;
...
constant
    : TIMESTAMP a_expr (AT TIME ZONE a_expr)?
    | INTERVAL a_expr (
        YEAR_P
        | MONTH_P
        | DAY_P
        ...
        | MINUTE_P
        | SECOND_P
    ) (OPEN_PAREN (numericonly | a_expr) (COMMA (numericonly | a_expr))? CLOSE_PAREN)? (
        TO (DAY_P | HOUR_P | MINUTE_P | SECOND_P (OPEN_PAREN (numericonly | a_expr) CLOSE_PAREN)?)
    )?
    | numeric
    | DATE a_expr
    | NULL_P
...
trigger_anonymous_block
    : (DECLARE seq_of_declare_specs)? BEGIN_P seq_of_statements (EXCEPTION exception_handler+)? END_P
    ;
...
create_package
    : CREATE (OR REPLACE)?  PACKAGE package_name invoker_rights_clause? (
        IS
        | AS
    ) package_obj_spec* END_P label_name?
    ;
...
create_package_body
    : CREATE (OR REPLACE)?  PACKAGE BODY  package_name (
        IS
        | AS
    ) package_obj_body*? (BEGIN_P seq_of_statements (EXCEPTION exception_handler+)?)? END_P label_name?
    ;
...
gaussdb_cleanconnection
    : CLEAN CONNECTION TO (
        COORDINATOR gaussdb_nodenamelist
        | NODE gaussdb_nodenamelist
        | ALL CHECK?  FORCE?
    )
    (   FOR DATABASE any_name
        | TO USER any_name
        | FOR DATABASE any_name TO USER any_name
    )
    ;
...
gaussdb_createpolicystmt:
	CREATE AUDIT POLICY (IF_P NOT EXISTS)? policy_name  gaussdb_audit_clause_list filter_group_clause?  ( ENABLE_P | DISABLE_P )?
;

gaussdb_audit_clause_list:
	privilege_access_audit_clause  (COMMA privilege_access_audit_clause )*
;

privilege_access_audit_clause:
    (PRIVILEGES | ACCESS)? ( dml_clause | ddl_clause | ALL ) ( ON LABEL OPEN_PAREN resource_label_name (COMMA resource_label_name)* CLOSE_PAREN )?
;

ddl_clause: ALTER | ANALYZE | COMMENT | CREATE | DROP | GRANT | REVOKE | SET | SHOW ;

resource_label_name:
    any_name
;

dml_clause: COPY | DEALLOCATE | DELETE_P | EXECUTE | REINDEX | INSERT | PREPARE | SELECT | TRUNCATE | UPDATE  ;

filter_group_clause:
    FILTER ON filter_type_list (COMMA filter_type_list)*
;

filter_type_list: filter_type  filter_value_list  ;

filter_type: APP | ROLES | IP   ;

filter_value_list:   OPEN_PAREN filter_value (COMMA filter_value)* CLOSE_PAREN ;

filter_value: a_expr  ;

gaussdb_createbarrierstmt:
    CREATE BARRIER sconst
;

gaussdb_create_client_master_key:
    CREATE CLIENT MASTER KEY client_master_key_name WITH client_master_key_list
;
...
client_master_key_list:
    OPEN_PAREN client_master_key (COMMA client_master_key)* CLOSE_PAREN
;

client_master_key:
    (KEY_STORE | KEY_PATH | ALGORITHM) EQUAL? a_expr
;

gaussdb_create_column_encryption_key:
    CREATE COLUMN ENCRYPTION KEY column_encryption_key_name WITH VALUES column_encryption_key_list
;
...
column_encryption_key_list:
    OPEN_PAREN column_encryption_key (COMMA column_encryption_key)* CLOSE_PAREN
;

column_encryption_key:
    (CLIENT_MASTER_KEY | ENCRYPTED_VALUE | ALGORITHM) EQUAL? a_expr
;
...
database_link_using:
    USING OPEN_PAREN database_link_using_opt (COMMA database_link_using_opt)* CLOSE_PAREN
;
...
gaussdb_create_directory
    : CREATE  opt_or_replace? DIRECTORY directory_name AS a_expr
;
...
gaussdb_create_masking_policy:
  CREATE  MASKING POLICY policy_name masking_clause_list  policy_filter_clause? (ENABLE_P | DISABLE_P)?
;

masking_clause_list:
  masking_clause (COMMA masking_clause)*
;

masking_clause:
  masking_function ON LABEL OPEN_PAREN label_name (COMMA label_name)* CLOSE_PAREN
;

//{ maskall | randommasking | creditcardmasking | basicemailmasking | fullemailmasking | shufflemasking | alldigitsmasking | regexpmasking }
masking_function:
  any_name (OPEN_PAREN a_expr (COMMA a_expr)* CLOSE_PAREN)?
;
...
gaussdb_create_node:
    CREATE NODE gaussdb_nodename WITH OPEN_PAREN colid EQUAL? a_expr (COMMA colid EQUAL? a_expr)* CLOSE_PAREN
;
...
gaussdb_create_node_group:
    CREATE NODE GROUP_P gaussdb_groupname WITH OPEN_PAREN gaussdb_nodename (COMMA gaussdb_nodename)* CLOSE_PAREN
    (BUCKETS (OPEN_PAREN gaussdb_bucketnumber (COMMA gaussdb_bucketnumber )* CLOSE_PAREN)? )?
;

gaussdb_bucketnumber:
    signediconst
;

gaussdb_create_resource_pool:
    CREATE RESOURCE POOL gaussdb_resource_pool_name WITH OPEN_PAREN colid EQUAL? a_expr (COMMA colid EQUAL? a_expr)* CLOSE_PAREN
;

gaussdb_resource_pool_name:
    any_name
;

gaussdb_create_resource_label:
    CREATE RESOURCE LABEL opt_if_not_exists? label_name ADD_P  label_item_list
;

label_item_list:
    label_item (COMMA label_item)*
;

label_item:
    resource_type OPEN_PAREN resource_path (COMMA resource_path)* CLOSE_PAREN
;
...
gaussdb_create_security_label:
    CREATE SECURITY LABEL opt_if_not_exists? label_name a_expr
;
...
gaussdb_partition_less_than_item_list
   : OPEN_PAREN gaussdb_partition_less_than_item (COMMA gaussdb_partition_less_than_item)* CLOSE_PAREN
   ;

gaussdb_partition_less_than_item
   : PARTITION any_name VALUES LESS THAN OPEN_PAREN ( MAXVALUE | a_expr ) (COMMA (MAXVALUE | a_expr))* CLOSE_PAREN  gaussdb_ilm_add_policy? opttablespace_list?
   ;


gaussdb_partition_start_end_item_list
   : OPEN_PAREN gaussdb_partition_start_end_item (COMMA gaussdb_partition_start_end_item)* CLOSE_PAREN
   ;

gaussdb_partition_start_end_item
   : PARTITION any_name (
          START OPEN_PAREN a_expr CLOSE_PAREN END_P OPEN_PAREN a_expr CLOSE_PAREN EVERY OPEN_PAREN a_expr CLOSE_PAREN
        | START OPEN_PAREN a_expr CLOSE_PAREN END_P OPEN_PAREN ( MAXVALUE | a_expr ) CLOSE_PAREN
        | START OPEN_PAREN a_expr CLOSE_PAREN
        | END_P OPEN_PAREN ( MAXVALUE | a_expr ) CLOSE_PAREN
   ) gaussdb_ilm_add_policy? opttablespace_list?
   ;

opttablespace_list:
    opttablespace (COMMA opttablespace)*
;

gaussdb_partition_hash_item
   : OPEN_PAREN PARTITION any_name  gaussdb_ilm_add_policy? opttablespace_list? CLOSE_PAREN
   ;

gaussdb_partition_list_item
   : OPEN_PAREN PARTITION any_name VALUES IN_P? OPEN_PAREN expr_list CLOSE_PAREN gaussdb_ilm_add_policy? opttablespace_list? CLOSE_PAREN
   ;

gaussdb_create_weak_password_dic
   : CREATE WEAK PASSWORD DICTIONARY (WITH VALUES)? OPEN_PAREN a_expr CLOSE_PAREN (COMMA OPEN_PAREN a_expr CLOSE_PAREN)*
   ;
...
gaussdb_replacestmt
   : REPLACE INTO? insert_target
   (PARTITION OPEN_PAREN gaussdb_partition_name (COMMA gaussdb_partition_name)* CLOSE_PAREN)?
   (insert_rest
...
gaussdb_show_events_stmt:
    SHOW EVENTS ((FROM | IN_P) optschemaname)? (LIKE a_expr | where_clause)?
;
//-------------------------------gaussdb独有语句 结束----------------------------